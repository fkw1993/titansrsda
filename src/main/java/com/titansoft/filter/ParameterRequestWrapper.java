...
class ParameterRequestWrapper extends HttpServletRequestWrapper {
	...
	public String getParameter(String name) {
		...
		try {
			if(result.indexOf("•")>=0)
			{
				result = specialCharDeal.specialCharEncode(result);
				result = specialCharDeal.sqlattCharsEncode(result);
			}
			/*if(result.indexOf("ttxssatt")>=0)
			{
				result = specialCharDeal.xssattCharsDecode(result);
			}*/
		} catch (Exception e) {
